...
function mdf(n) {
    ...
    opnDiv("addWrd", "nwEn", "nwAr"), gebi("pAdd").innerText = "edit words", gebi("okAdd").innerText = "edit", gebi("nmW").innerText = n, e("En", enWrd[n]), e("Fr", frWrd[n]), e("Ar", arWrd[n]), wrdEdt = !0,chcInp();
...
function resulte() {
    ...
    gebi("tmLft").innerText = clTm(lftTm), dsply("cntnr", "block"), nbWrd > 0 && void 0 == n || nbWrd > n ? (cngrt = `<div>congratulations <span> ${e}</span></div><div>You wrote <span> ${nbWrd}</span> words in <span> ${clTm(lftTm, 1)}</span>
...
function trnslt() {
    ...
    if (arLng + enLng + frLng == '') {
        root.style.setProperty('--animation', 'animTrans 2s linear 1');
        setTimeout(() => {
            root.style.setProperty('--animation', '');
        }, 2200);
    } else {
        if (arLng.length) {
    ...
    function tr(vl, lng1, lng2, elRslt) {
        if (elRslt.style.display =='none') {return false}
...
function fedt() {
    gebi("pAdd").innerText = "add New words", gebi("okAdd").innerText = "add", vidInp(), wrdEdt = !1
}

function addWrds() {
    if ("add New words" != rInrTxt("pAdd")) {
        edt()
        return false
    }
...
/* 
    slctAll('.arSound').forEach(e => {
        e.onmousedown = (ev) => {
            ev.preventDefault();
            let lng = gebi(e.dataset.lng), txt = lng.innerText;
            if (txt) {
                selectText(e.dataset.lng);
                setTimeout(() => {
                    lng.innerText = '';
                    lng.innerText = txt;
                }, 1000);
 ...
 */
    ...
    let soundLong = { en: "", fr: "" }, synth = speechSynthesis;
    ...
    slctAll('.soundFrEn').forEach(e => {
        e.onclick = (ev) => {
            ...
            let text = gebi(e.dataset.lng).innerText, collection= e.children;

            if (text) {
                utterance = new SpeechSynthesisUtterance(text);
                utterance.voice = soundLong[e.dataset.voi];
    ...
    slctAll('.sound pose').forEach(e=>{
        e.onclick = ()=>{
        if (synth.speaking) {
            e.classList.toggle('actice');
            e.previousElementSibling.classList.toggle('actice');
            synth.pause();
        }
        }
    }) */
    slctAll('.sound pose').forEach(e=>{
        if (synth.speaking) {
            slctAll('.sound svg').forEach(e => {e.classList.toggle("active");});
            synth.pause();
        }
    })
...
function cntn() {
    ...
    enWrd = r, arWrd = t, frWrd = i, gebi("nmbWrd").innerText = enWrd.length, slctAll(".ch input").forEach(n => {
        let e = n.value;
...
/* gebi('button').onclick = () => {
    ...
    let sourceText = gebi('sourceText').value, sourceLang = 'En', targetLang = 'Ar';
    console.log(encodeURI(sourceText));

    var url = "https://api.mymemory.translated.net/get?q=" + sourceText +
        "&langpair=" + sourceLang + '|' + targetLang;
    //console.log(url);
    fetch(url).then(n => n.json()).then(data => {
        gebi('resultText').value = data.responseData.translatedText
    });
